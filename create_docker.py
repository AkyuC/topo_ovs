...
if __name__ == '__main__' :
    ...
    for index in range(sw_num):
        ...
        os.system("sudo docker create --name=s{} --net=none --privileged ovs".format(index))
        os.system("sudo docker create --name=c{} --net=none --privileged openmul".format(index))
    ...
    with open(file=filePath + "/config/rt_ctrl2db/db_deploy") as file:
        ...
        for db in db_data:
            os.system("sudo docker create --name=db{} --net=none --privileged database".format(db))