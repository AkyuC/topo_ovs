    ...
    def change_slot_ctrl(cslot_b:dict, cslot_n:dict):
        ...
        for ctrl in cslot_b:
            if ctrl not in cslot_n:
                os.system("sudo docker exec -it s{} ovs-ofctl del-flow s{} \"cookie=0,priority=2,ip,nw_dst=192.168.67.{} action=output:{}\""\
                    .format(ctrl, ctrl, ctrl+1, ctrl+3000))
                os.system("sudo docker exec -it s{} ovs-ofctl del-flow s{} \"cookie=0,priority=2,arp,nw_dst=192.168.67.{} action=output:{}\""\
                    .format(ctrl, ctrl, ctrl+1, ctrl+3000))
                os.system("sudo docker exec -it s{} ovs-vsctl del-port s{} s{}-c{}".format(ctrl, ctrl, ctrl, ctrl))
